...
function readPositiveInteger(name, maximum) {
  ...
  if (
    ...
    throw new Error(
      `Konfigurasi ${name} harus bilangan bulat antara 1 dan ${maximum}.`
    );
  }
...
function readSslConfig() {
  const mode = readRequired('PGSSLMODE').trim();
  ...
  throw new Error('PGSSLMODE harus disable atau verify-full.');
}
