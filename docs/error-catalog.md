...
| `https://api.titipin.example/problems/authentication-required` | 401 | Authentication required | Token tidak ada, signature tidak valid, kedaluwarsa, atau issuer/audience tidak sesuai. | Tidak ada. | Hentikan request, perbarui autentikasi, lalu ulangi hanya setelah autentikasi berhasil. |
| `https://api.titipin.example/problems/forbidden` | 403 | Action is not permitted | Token valid, tetapi scope operasi kurang; ditolak sebelum query objek. | `requiredPermission`. | Sembunyikan tindakan yang tidak tersedia dan jangan melakukan retry otomatis. |
| `https://api.titipin.example/problems/resource-not-found` | 404 | Resource not found | Resource dengan identifier pada path tidak ditemukan atau tidak boleh diungkapkan kepada aktor tersebut. | Tidak ada identifier objek; body identik untuk tidak ada dan tidak berhak. | Hentikan retry dan muat ulang koleksi atau navigasikan pengguna kembali. |
